...
runTest() {
  ...
  echo "Test $TESTNO: ./rental ${args[@]} < input-$TESTNO.txt > output.txt 2> stderr.txt"
  ./rental ${args[@]} < input-$TESTNO.txt > output.txt 2> stderr.txt
  STATUS=$?
...
if [ -x rental ] ; then
    ...
    args=(houses-a.txt houses-b.txt)
    runTest 03 0
    
    args=(houses-b.txt houses-c.txt)
    runTest 04 0
    ...
    args=(houses-a.txt houses-b.txt houses-c.txt)
    runTest 13 0
    ...
    args=(houses-a.txt file-that-doesnt-exist.txt)
    runTest 16 1
